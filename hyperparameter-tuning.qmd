...
#tune was removed to set better metrics for tuning.
...
```{r, echo = TRUE}
dials <- extract_parameter_set_dials(rf_wf3) %>% 
...
rf_model_tune <- rand_forest(
  ...
  set_engine("ranger", importance = "impurity", seed = 123) %>%
  set_mode("regression") %>%
  # Update the default parameter ranges:
  update(
    mtry = mtry(range = c(1, 10)),       # Custom range for mtry
    trees = trees(range = c(200, 800)),  # Custom range for number of trees
    min_n = min_n(range = c(5, 15))      # Custom range for minimum node size
  )

```

## Updating the worflow
...
dials_params <- extract_parameter_set_dials(rf_wf3_tune) %>% 
  update(
    mtry = mtry(range = c(1, 13)),  # Now properly constrained to 13
...
## Updating Latin Hypercube Grid

```{r, echo = TRUE}
set.seed(123)
my.grid <- grid_latin_hypercube(
  dials_params,  # Using the UPDATED parameters
  size = 20
)
```
...
```{r, echo = TRUE}
n_predictors <- ncol(camels_train3) - 1  # Exclude outcome variable
print(paste("Number of predictors:", n_predictors))
print(my.grid)
```

```{r, echo = TRUE}
full_params <- tune_grid(
  rf_wf3_tune,
  resamples = camels_cv3,
  grid = my.grid,
  metrics = metric_set(rmse, rsq, mae),
...
full_params %>% 
  collect_metrics() %>%
  filter(.metric == "rsq") %>%
  slice_max(n = 5, order_by = mean)

full_params %>% 
  collect_metrics() %>% 
  filter(.metric == "rmse") %>% 
  slice_min(n = 5, order_by = mean)
...
The combination with the highest r-squared is .92. Its min_n is 6 and it has 376 trees, which is by no means the most trees or the most min_n. The next four model combinations also have a mean of .92 rsq. The range of rmse is from .41 to .43.  The range of mae is .22 to .25. The lowest rmse has 396 trees and a min_n of 12. In this situation, I can see how it's helpful to graph all of these metrics to look for trends. The autoplot seems more helpful in this situaiton, while the table is for finding a specific combo that worked best.

### Using select_best()

``` {r, echo = TRUE}
show_best(full_params, metric = "mae", n = 5)
```
...
The best model, measured by minimal mae, had six randomly sampled errors, 227 trees, and seven samples. 

## select_best hyperparameter
``` {r, echo = TRUE}
hp_best <- select_best(full_params, metric = "mae")
```
## Finalizing the workflow now that we have the best hyperparameter specs
``` {r, echo = TRUE}
finalized_workflow <- finalize_workflow(rf_wf3_tune, select_best(full_params, metric = "mae"))

```

## Final Fitting
``` {r, echo = TRUE}
final_fit <- last_fit(finalized_workflow, camels_split3) 
...
```{r, echo = TRUE}
final_final_fit <- fit(finalized_workflow, camels)


final_final_augment <- augment(final_final_fit, camels) %>% 
...
predmap <- ggplot(data = final_final_augment, aes(x = gauge_lon, y = gauge_lat)) +
...
# Residuals Map
resid <- ggplot(data = final_final_augment, aes(x = gauge_lon, y = gauge_lat)) +
  borders("state", colour = "gray50") +
